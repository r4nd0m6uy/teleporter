...
local function build_destination_drop_list(source_hash)
        ...
        for dest_hash, dest_tp in pairs(db) do
                ...
                then
                        ...
                        if db[source_hash].destination_hash == dest_hash
                        then
...
local function update_teleporters_meta()
        ...
        for hash, tp in pairs(db) do
                ...
                if tp.destination_hash ~= nil and db[tp.destination_hash] == nil
                then
...
local function teleporter_configured(pos, formname, fields, sender)
        ...
        -- Name was changed
        if newName ~= nil and newName ~= "" and newName ~= teleporter.name
        then
                if teleporter_name_exists(newName)
                then
        ...
        if newDest ~= nil and newDest ~= teleporter.destination_name
        then                
                if newDest == "" then
